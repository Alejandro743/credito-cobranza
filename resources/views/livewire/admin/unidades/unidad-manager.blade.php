...
<div class="flex flex-col sm:flex-row gap-3 mb-4">
    <div class="relative flex-1">
        ...
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Buscar por código o nombre..."
               class="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-mint-400 focus:ring-2 focus:ring-mint-100">
    </div>
    <select wire:model.live="filterStatus" class="border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-mint-400 bg-white">
        <option value="">Todos los estados</option>
        <option value="1">Activas</option>
        <option value="0">Inactivas</option>
    </select>
    <button wire:click="showAdd"
            class="flex items-center gap-2 bg-mint-500 hover:bg-mint-600 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition-colors whitespace-nowrap">
...
@if ($showAddForm)
<div class="bg-mint-50 border border-mint-200 rounded-2xl p-4 mb-4">
    <p class="text-xs font-bold text-mint-700 uppercase tracking-wide mb-3">Nueva Unidad</p>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
            <input wire:model="newName" type="text" placeholder="Nombre de la unidad *"
                   class="w-full border border-mint-200 bg-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-mint-400 focus:ring-2 focus:ring-mint-100">
            @error('newName') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>
        <div>
            <input wire:model="newAbreviatura" type="text" maxlength="20" placeholder="Abreviatura (ej: kg)"
                   class="w-full border border-mint-200 bg-white rounded-xl px-3 py-2 text-sm font-mono focus:outline-none focus:border-mint-400 focus:ring-2 focus:ring-mint-100">
            @error('newAbreviatura') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>
    </div>
    <div class="flex items-center gap-3 mt-3">
        <label class="flex items-center gap-2 cursor-pointer text-sm text-gray-700">
            <input type="checkbox" wire:model="newActive" class="w-4 h-4 rounded text-mint-500 border-gray-300">
            Activa
        </label>
        <div class="flex-1"></div>
        <button wire:click="cancelAdd" class="px-4 py-2 text-sm text-gray-600 hover:bg-mint-100 rounded-xl transition-colors font-medium">
            Cancelar
        </button>
        <button wire:click="saveNew" class="px-5 py-2 text-sm font-semibold bg-mint-500 hover:bg-mint-600 text-white rounded-xl transition-colors">
            Guardar
        </button>
    </div>
</div>
        ...
        <table class="w-full text-sm">
            <thead class="{{ $theadClass }} text-xs uppercase tracking-wide">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide hidden sm:table-cell">Abreviatura</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr wire:key="u-edit-{{ $u->id }}" class="bg-mint-50 border-l-2 border-mint-400">
                    <td class="px-4 py-2 font-mono text-xs text-gray-400">auto</td>
                    <td class="px-4 py-2">
                        <input wire:model="editName" type="text" placeholder="Nombre *"
                               class="w-full border border-mint-300 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:border-mint-500 focus:ring-2 focus:ring-mint-100 bg-white">
                    ...
                    <td class="px-4 py-2 hidden sm:table-cell">
                        <input wire:model="editAbreviatura" type="text" maxlength="20" placeholder="kg"
                               class="w-24 border border-mint-300 rounded-lg px-2.5 py-1.5 text-sm font-mono focus:outline-none focus:border-mint-500 bg-white">
                    </td>
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-end gap-1">
                            ...
                            <button wire:click="toggleActive({{ $u->id }})" title="{{ $u->active ? 'Desactivar' : 'Activar' }}"
                                    ...
                                    {{ $u->active ? 'text-gray-400 hover:text-melocoton-600 hover:bg-melocoton-50' : 'text-gray-400 hover:text-mint-600 hover:bg-mint-50' }}">
                                @if ($u->active)
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
