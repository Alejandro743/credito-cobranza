...
<div class="hidden sm:block" style="background:#fff; border-radius:16px; border:1px solid #E5E7EB; box-shadow:0 1px 4px rgba(0,0,0,.06); overflow:hidden; display:flex; flex-direction:column; max-height:calc(100vh - 180px);">
    ...
    <div style="padding:10px 18px; display:flex; align-items:center; border-bottom:1px solid #F3F4F6; flex-shrink:0;">
        ...
        <div style="display:flex; align-items:center; gap:5px; margin-left:10px; padding-left:10px; border-left:1px solid #E5E7EB;">
            ...
            @else
                <button wire:click="startEdit({{ $selectedItemId }})" style="{{ $btnH }} border:1px solid #EDE9FE; background:#F8F7FF; color:#7B6FE8; transition:background .15s, color .15s;" onmouseenter="this.style.background='#7B6FE8'; this.style.color='#fff';" onmouseleave="this.style.background='#F8F7FF'; this.style.color='#7B6FE8';">
                ...
                <button wire:click="openStockModal({{ $selectedItemId }})" style="{{ $btnH }} border:1px solid #EDE9FE; background:#F8F7FF; color:#7B6FE8; transition:background .15s, color .15s;" onmouseenter="this.style.background='#7B6FE8'; this.style.color='#fff';" onmouseleave="this.style.background='#F8F7FF'; this.style.color='#7B6FE8';">
                    <svg width="10" height="10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2M9 12h6M9 16h4"/></svg>
                    Ver Listas
                </button>
            @endif
        </div>
        ...
        <table style="width:100%; min-width:1000px; border-collapse:collapse; font-size:13px;">
            ...
            <tbody>
                ...
                <tr wire:key="item-{{ $item->id }}"
                    ...
                    <td style="padding:10px 14px; text-align:right;">
                        <span style="font-size:12px; color:#374151;">{{ number_format((float)$item->stock_inicial, 0) }}</span>
                    </td>

                    <td style="padding:10px 14px; text-align:right;">
                        <span style="font-size:12px; color:#374151;">{{ number_format((float)$item->stock_actual, 0) }}</span>
                    </td>

                    <td style="padding:10px 14px; text-align:right;">
                        <span style="font-size:12px; color:#374151;">{{ number_format((float)$item->stock_comprometido, 0) }}</span>
                    </td>

                    <td style="padding:10px 14px; text-align:right;">
                        @php $disp = $item->stockDisponible(); @endphp
                        <span style="font-size:12px; color:#374151;">{{ number_format($disp, 0) }}</span>
                    </td>
...
<div style="position:fixed; inset:0; z-index:200; display:flex; align-items:center; justify-content:center; padding:16px; background:rgba(0,0,0,.45);"
    ...
    <div style="background:#fff; border-radius:20px; width:100%; max-width:640px; max-height:88vh; display:flex; flex-direction:column; box-shadow:0 24px 64px rgba(0,0,0,.22);">
        ...
        <div style="padding:16px 20px; border-bottom:1px solid #EDE9FE; display:flex; align-items:center; gap:10px; flex-shrink:0;">
            ...
            <div style="flex:1; min-width:0;">
                <p style="font-size:13px; font-weight:800; color:#7B6FE8; margin:0;">Listas de precios que usan este artículo</p>
                <p style="font-size:12px; color:#9CA3AF; margin:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                    ...
                    &nbsp;·&nbsp;Ciclo: {{ $stockModalItem?->listaMaestra?->cycle?->code }}
                </p>
            </div>
            ...
            <table style="width:100%; border-collapse:collapse; font-size:13px;">
                <thead>
                    <tr style="background:#F9F8FF; border-bottom:2px solid #EDE9FE;">
                        ...
                        <th style="padding:9px 12px; text-align:right; font-size:11px; font-weight:700; color:#7B6FE8; text-transform:uppercase; letter-spacing:.4px;">Stock Asig.</th>
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid #F3F4F6;">
                        <td style="padding:10px 12px;">
                            <span style="font-size:13px; color:#374151; font-weight:600;">{{ $row->listaDerivada?->name ?? '—' }}</span>
                        </td>
                        <td style="padding:10px 12px; text-align:right;">
                            <span style="font-size:13px; font-weight:700; color:#7B6FE8; font-family:monospace;">{{ number_format((float)$row->stock_asignado, 0) }}</span>
                        </td>
        ...
        <div style="padding:14px 20px; border-top:1px solid #EDE9FE; display:flex; justify-content:flex-end; flex-shrink:0;">
            <button wire:click="closeStockModal"
                    style="height:36px; padding:0 24px; border:1px solid #EDE9FE; background:#F8F7FF; color:#7B6FE8; border-radius:9px; font-size:13px; font-weight:700; cursor:pointer; transition:background .15s, color .15s;"
                    onmouseenter="this.style.background='#7B6FE8'; this.style.color='#fff';" onmouseleave="this.style.background='#F8F7FF'; this.style.color='#7B6FE8';">
                Cerrar
            </button>
        </div>
