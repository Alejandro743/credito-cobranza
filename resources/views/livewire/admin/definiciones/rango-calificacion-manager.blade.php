...
$umbrales = [
    ['A – Excelente', 'A', '#D1FAE5', '#059669'],
    ['B – Buena',     'B', '#CFFAFE', '#0E7490'],
    ['C – Observada', 'C', '#FEF3C7', '#B45309'],
    ['D – Riesgosa',  'D', '#FFEDD5', '#C2410C'],
];
@endphp
...
<div class="hidden sm:block" style="background:#fff;border-radius:16px;border:1px solid #EDE9FE;box-shadow:0 2px 12px rgba(123,111,232,.12);margin-bottom:20px;overflow:hidden;">
    ...
    <div style="padding:16px 20px;display:flex;align-items:flex-start;gap:12px;flex-wrap:wrap;">
        <div style="min-width:200px;">
            ...
            <input wire:model="newNombre" type="text" placeholder="Ej: Rangos 2026" style="width:100%;{{ $iS }}">
    ...
    <div style="padding:0 20px 16px;">
        <div style="border:1px solid #EDE9FE;border-radius:10px;padding:14px 16px;background:#FAFAFE;">
            ...
            @error('newMinA')<p style="font-size:11px; color:#EF4444; margin:-6px 0 10px;">{{ $message }}</p>@enderror
            <div style="display:flex;align-items:flex-start;gap:12px;flex-wrap:wrap;">
                @foreach($umbrales as [$lbl, $letra, $bg, $cl])
                <div style="min-width:120px;">
                    <label style="font-size:11px; font-weight:700; color:{{ $cl }}; text-transform:uppercase; letter-spacing:.5px; margin-bottom:5px; display:block;">{{ $lbl }}</label>
                    <div style="position:relative;">
                        <input wire:model.live="newMin{{ $letra }}" type="number" min="0" max="100" step="0.5"
                               style="width:100%;height:38px;border:1.5px solid {{ $bg }};border-radius:8px;padding:0 30px 0 10px;font-size:14px;font-weight:700;outline:none;background:{{ $bg }};color:{{ $cl }};box-sizing:border-box;">
...
<div class="sm:hidden" style="background:#FAFAFE;border-radius:14px;border:1px solid #EDE9FE;margin-bottom:16px;padding:14px 16px;box-shadow:0 1px 4px rgba(123,111,232,.1);">
    ...
    <div style="border:1px solid #EDE9FE;border-radius:10px;padding:12px;background:#fff;margin-bottom:12px;">
        <div style="display:flex; align-items:center; gap:10px; margin-bottom:10px;">
            <span style="font-size:11px; font-weight:700; color:#7B6FE8; text-transform:uppercase; letter-spacing:.5px;">Umbrales</span>
            <div style="flex:1; height:1px; background:#EDE9FE;"></div>
        </div>
        @error('newMinA')<p style="font-size:10px; color:#EF4444; margin:-6px 0 10px;">{{ $message }}</p>@enderror
        <div style="display:flex;flex-direction:column;gap:10px;">
            @foreach($umbrales as [$lbl, $letra, $bg, $cl])
            <div>
                <label style="font-size:11px; font-weight:700; color:{{ $cl }}; text-transform:uppercase; letter-spacing:.5px; margin-bottom:3px; display:block;">{{ $lbl }}</label>
                <div style="position:relative;">
                    <input wire:model.live="newMin{{ $letra }}" type="number" min="0" max="100" step="0.5"
                           style="width:100%;height:36px;border:1.5px solid {{ $bg }};border-radius:8px;padding:0 30px 0 10px;font-size:14px;font-weight:700;outline:none;background:{{ $bg }};color:{{ $cl }};box-sizing:border-box;">
                    <span style="position:absolute; right:10px; top:50%; transform:translateY(-50%); font-size:12px; color:{{ $cl }}; font-weight:700; opacity:.6;">pts</span>
                </div>
            </div>
            @endforeach
...
<div class="hidden sm:block" style="background:#fff;border-radius:16px;border:1px solid #E5E7EB;box-shadow:0 1px 4px rgba(0,0,0,.06);overflow:hidden;display:flex;flex-direction:column;max-height:calc(100vh - 180px);">
    ...
    <div style="padding:10px 18px;display:flex;align-items:center;gap:8px;border-bottom:1px solid #F3F4F6;flex-shrink:0;">
        ...
        <div style="display:flex; align-items:center; gap:5px; margin-left:4px; padding-left:10px; border-left:1px solid #E5E7EB;">
            ...
            @else
                <button wire:click="startEdit({{ $selectedRangoId }})" style="{{ $btnH }} border:1px solid #EDE9FE; background:#F8F7FF; color:#7B6FE8; transition:background .15s, color .15s;" onmouseenter="this.style.background='#7B6FE8'; this.style.color='#fff';" onmouseleave="this.style.background='#F8F7FF'; this.style.color='#7B6FE8';">
        ...
        <table style="width:100%;border-collapse:collapse;min-width:1050px;">
                ...
                <tr style="background:#F9F8FF;border-bottom:2px solid #EDE9FE;">
                    <th style="width:50px; padding:8px 8px 6px; text-align:center; font-size:11px; font-weight:700; color:#C4B5FD; text-transform:uppercase; letter-spacing:.5px; white-space:nowrap; vertical-align:top; position:sticky; left:0; z-index:11; background:#F9F8FF;">
                        ...
                        <div style="margin-top:4px; height:28px; display:flex; align-items:center; justify-content:center;">
                            <input type="checkbox"
                                   :checked="$wire.selectedRangoId !== null"
                                   :disabled="$wire.selectedRangoId === null || $wire.editingId !== null"
                                   @click.prevent="$wire.selectedRangoId !== null && $wire.editingId === null && $wire.set('selectedRangoId', null)"
                                   :style="($wire.selectedRangoId !== null && $wire.editingId === null) ? 'accent-color:#7B6FE8; width:15px; height:15px; cursor:pointer;' : 'accent-color:#7B6FE8; width:15px; height:15px; cursor:default; opacity:0.35;'">
                        </div>
                    </th>
                    ...
                    {{-- D --}}
                    @php $isA = $sortBy === 'min_d'; @endphp
                    <th wire:click="toggleSort('min_d')" style="{{ $thC }} text-align:center; cursor:pointer; min-width:90px; {{ $isA ? 'background:#EDE9FE;' : '' }}"
                        @mouseenter="!{{ $isA?'true':'false' }} && ($el.style.background='#F5F3FF')" @mouseleave="!{{ $isA?'true':'false' }} && ($el.style.background='')">
                        <div style="display:flex; align-items:center; justify-content:center; gap:4px;">D
                            <span style="display:inline-flex; flex-direction:column; gap:1px; line-height:1;">
                                <svg width="7" height="7" viewBox="0 0 10 6" fill="{{ $isA && $sortDir==='asc' ? '#7B6FE8':'#C4B5FD' }}"><path d="M5 0l5 6H0z"/></svg>
                                <svg width="7" height="7" viewBox="0 0 10 6" fill="{{ $isA && $sortDir==='desc' ? '#7B6FE8':'#C4B5FD' }}"><path d="M5 6l5-6H0z"/></svg>
                            </span>
            ...
            <tbody>
                ...
                <tr wire:key="edit-err-{{ $r->id }}" style="background:#FAFAFE;">
                    <td colspan="10" style="padding:0 10px 10px; position:sticky; left:0;">
                ...
                @else
                {{-- Fila normal --}}
                @php $selR = $selectedRangoId === $r->id; $esVigente = $vigenteId === $r->id; $esAbierta = $r->fecha_fin === null; @endphp
                <tr wire:key="rc-{{ $r->id }}"
                    ...
                    <td class="col-row-num" style="padding:6px 6px; text-align:center; position:sticky; left:0; z-index:2; background:{{ $selR ? '#F5F3FF' : '#fff' }};">
                        <div style="display:inline-flex; align-items:center; justify-content:center; gap:6px;">
                            @if($esAbierta)
                            <input type="checkbox"
                                   :checked="$wire.selectedRangoId === {{ $r->id }}"
                                   @click="$wire.selectedRangoId === {{ $r->id }} ? $wire.set('selectedRangoId', null) : $wire.selectRango({{ $r->id }})"
...
<div class="sm:hidden flex flex-col" style="gap:10px;">
    ...
    <div wire:key="card-edit-{{ $r->id }}"
        ...
        <div style="border:1px solid #EDE9FE;border-radius:10px;padding:12px;background:#fff;margin-bottom:12px;">
            ...
            @error('editMinA')<p style="font-size:10px; color:#EF4444; margin:-6px 0 10px;">{{ $message }}</p>@enderror
            <div style="display:flex;flex-direction:column;gap:10px;">
                @foreach($umbrales as [$lbl, $letra, $bg, $cl])
                <div>
                    <label style="font-size:11px; font-weight:700; color:{{ $cl }}; text-transform:uppercase; letter-spacing:.5px; margin-bottom:3px; display:block;">{{ $lbl }}</label>
                    <div style="position:relative;">
                        <input wire:model.live="editMin{{ $letra }}" type="number" min="0" max="100" step="0.5"
                               style="width:100%;height:36px;border:1.5px solid {{ $bg }};border-radius:8px;padding:0 30px 0 10px;font-size:14px;font-weight:700;outline:none;background:{{ $bg }};color:{{ $cl }};box-sizing:border-box;">
                        <span style="position:absolute; right:10px; top:50%; transform:translateY(-50%); font-size:12px; color:{{ $cl }}; font-weight:700; opacity:.6;">pts</span>
                    </div>
                </div>
    ...
    <div wire:key="card-{{ $r->id }}"
        ...
        <div style="padding:12px 14px;display:flex;flex-direction:column;gap:8px;">
            @foreach([
                ['A — Excelente', $r->min_a, '#D1FAE5', '#059669'],
                ['B — Buena',     $r->min_b, '#CFFAFE', '#0E7490'],
                ['C — Observada', $r->min_c, '#FEF3C7', '#B45309'],
                ['D — Riesgosa',  $r->min_d, '#FFEDD5', '#C2410C'],
            ] as [$lbl, $min, $bg, $cl])
            <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
                <span style="font-size:12px; color:#6B7280; flex:1;">{{ $lbl }}</span>
                <span style="font-size:13px; font-weight:800; padding:3px 12px; border-radius:99px; background:{{ $bg }}; color:{{ $cl }}; white-space:nowrap;">≥ {{ $min }} pts</span>
            </div>
            @endforeach
            <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
                <span style="font-size:12px; color:#6B7280; flex:1;">Bloqueado</span>
                <span style="font-size:13px; font-weight:800; padding:3px 12px; border-radius:99px; background:#FEE2E2; color:#DC2626; white-space:nowrap;">&lt; {{ $r->min_d }} pts</span>
            </div>
        </div>
