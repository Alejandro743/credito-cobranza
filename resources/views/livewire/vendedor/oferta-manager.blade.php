<div x-data="{ toastShow: false, toastMsg: '', showSearch: false, showProductos: false, ubEntModal: false, ubEntTipo: '', ubEntOpciones: [], ubEntSearch: '', appToastShow: false, appToastMsg: '', appToastType: 'success' }"
     x-effect="document.body.style.overflow = (showProductos || ubEntModal) ? 'hidden' : ''; if (!showSearch && !$wire.clienteId) $wire.set('searchCliente', '')"
...
<div class="hidden md:block bg-white px-4 pt-2 pb-2.5" style="max-width:900px; margin:0 auto;">
    ...
    <div class="flex gap-3 items-stretch" style="min-height:52px;">
        {{-- CLIENTE --}}
        <div style="flex:1; min-width:0; position:relative;" @click.outside="showSearch = false">
            @if ($sinListasActivas)
            <div style="background:#fff; border:1px solid #EDE9FE; border-radius:10px; box-shadow:0 2px 8px rgba(123,111,232,0.08); display:flex; align-items:center; gap:12px; padding:0 16px; min-height:52px;">
            ...
            <div style="position:relative;">
                ...
                @if ($clienteId)
                <div wire:key="cliente-sel-desktop-{{ $clienteId }}">
                ...
                @else
                <div wire:key="cliente-search-desktop">
                ...
                @endif
                <div x-show="showSearch && $wire.searchCliente.length >= 2" x-cloak
                     x-transition:enter="transition ease-out duration-100"
                     x-transition:enter-start="opacity-0 -translate-y-1"
                     x-transition:enter-end="opacity-100 translate-y-0"
                     style="position:absolute; left:0; right:0; top:calc(100% + 4px); background:#fff; border:1px solid #E5E7EB; border-radius:10px; box-shadow:0 4px 16px rgba(0,0,0,.1); z-index:60; max-height:220px; overflow-y:auto;">
                    @include('livewire.vendedor.partials.cliente-resultados')
                </div>
                </div>
                @endif
            </div>
...
<div class="md:hidden bg-white px-2 pt-2 pb-2" style="max-width:900px; margin:0 auto;">
    ...
    <div class="flex gap-1.5 items-stretch" style="min-height:44px;">
        ...
        <div style="flex:1; min-width:0; position:relative;" @click.outside="showSearch = false">
            @if ($sinListasActivas)
            <div style="background:#fff; border:1px solid #EDE9FE; border-radius:8px; box-shadow:0 2px 8px rgba(123,111,232,0.08); display:flex; align-items:center; gap:8px; padding:6px 10px;">
            ...
            <div style="position:relative;">
                ...
                @if ($clienteId)
                <div wire:key="cliente-sel-mobile-{{ $clienteId }}">
                ...
                @else
                <div wire:key="cliente-search-mobile">
...
<style>
.art-grid, .art-grid-row { display:grid; grid-template-columns:40px minmax(0,1fr) 68px 88px 54px 34px; align-items:stretch; }
@media (max-width:480px) {
    .art-grid, .art-grid-row { grid-template-columns:36px minmax(0,1fr) 34px; }
    .art-grid .art-precio, .art-grid .art-total, .art-grid .art-pts,
    .art-grid-row .art-precio, .art-grid-row .art-total, .art-grid-row .art-pts { display:none !important; }
    .art-mobile-stats { display:flex !important; }
}
</style>
...
<div style="padding:16px 16px 24px; max-width:900px; margin:0 auto;">
    ...
    <div style="background:#fff; border:1.5px solid #C4B5FD; border-radius:14px; overflow:hidden; box-shadow:0 2px 4px rgba(60,52,137,0.06), 0 8px 20px rgba(60,52,137,0.08);">
        ...
        @foreach ($carrito as $pid => $item)
            ...
            @endphp
            <div class="art-grid-row" wire:key="sel-{{ $pid }}">
        ...
        <div style="position:fixed; left:0; right:0; bottom:0; z-index:400; background:#fff; border-top:1px solid #F0EEFF; padding:10px 16px; box-shadow:0 -4px 16px rgba(60,52,137,0.08);">
        <div style="display:flex; gap:10px; max-width:900px; margin:0 auto;">
            <button wire:click="cambiarCliente"
                    style="flex:1; padding:13px 10px; background:#F8F7FF; color:#7B6FE8; font-size:13px; font-weight:700; border-radius:16px; border:1.5px solid #EDE9FE; cursor:pointer; -webkit-appearance:none; appearance:none; display:flex; align-items:center; justify-content:center; gap:6px; transition:background .15s, color .15s;"
            ...
            <button wire:click="confirmarPedido"
                    style="flex:2; padding:13px 10px; background:#F8F7FF; color:#7B6FE8; font-size:13px; font-weight:800; border-radius:16px; border:1.5px solid #EDE9FE; cursor:pointer; -webkit-appearance:none; appearance:none; display:flex; align-items:center; justify-content:center; gap:6px; transition:background .15s, color .15s;"
                    onmouseenter="this.style.background='#7B6FE8'; this.style.color='#fff';" onmouseleave="this.style.background='#F8F7FF'; this.style.color='#7B6FE8';">
...
<div x-data="{ staged: {}, stagedTotal() { return Object.values(this.staged).reduce((a,b) => a + b, 0); } }"
     x-show="showProductos" x-cloak
     x-transition:enter="transition ease-out duration-200"
     x-transition:enter-start="opacity-0"
     x-transition:enter-end="opacity-100"
     x-transition:leave="transition ease-in duration-150"
     x-transition:leave-end="opacity-0"
     class="fixed inset-0 flex flex-col modal-productos-outer"
     @click.self="staged = {}; showProductos = false"
    ...
    <div class="flex flex-col flex-1 w-full modal-productos-panel"
        ...
        <div style="max-width:900px; margin:0 auto;">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;">
                <div style="display:flex; align-items:center; gap:9px;">
                    ...
                    <p style="font-size:17px; font-weight:700; color:#3C3489; margin:0; letter-spacing:-0.2px;">Seleccionar Artículos</p>
                </div>
                <button @click="showProductos = false" type="button"
                ...
                <div style="display:flex; align-items:center; gap:10px;">
                    ...
                    <div style="position:relative; flex:1;">
                        ...
                        <input wire:model.live.debounce.300ms="searchProducto" type="text"
                               ...
                               style="width:100%; padding:8px 10px 8px 28px; background:#FAFAFE; border:1.5px solid #EDE9FE; border-radius:10px; font-size:12px; color:#3C3489; outline:none; box-sizing:border-box; transition:border-color 0.15s, background 0.15s;"
        ...
        <div style="max-width:900px; margin:0 auto;">
            <div style="display:flex; flex-direction:column; gap:8px;">
                ...
                @php $iid2=(string)$p['item_id']; $stock2=(int)$p['stock']; @endphp
                <div x-data="{ maxStock: @js($stock2), get qty() { return this.staged['{{ $iid2 }}'] || 0; } }"
                    ...
                    <div style="display:flex; align-items:center; gap:6px;">
                        ...
                        <button @click="staged['{{ $iid2 }}'] = Math.min((staged['{{ $iid2 }}']||0) + 1, maxStock)"
                                class="transition-all duration-150 hover:bg-[#F5F3FF] hover:shadow-md active:scale-90"
                                style="background:#fff; color:#7B6FE8; border:1.5px solid #7B6FE8; border-radius:7px; padding:5px 10px; font-size:11px; font-weight:700; cursor:pointer; -webkit-appearance:none; appearance:none; display:flex; align-items:center; justify-content:center; gap:4px; flex-shrink:0;">
                            <svg style="width:11px; height:11px; flex-shrink:0;" fill="none" stroke="#7B6FE8" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                            Agregar
                        </button>
                        <button x-show="qty > 0" x-cloak
                                @click="delete staged['{{ $iid2 }}']"
                                class="btn-circle-center transition-all duration-150 hover:bg-[#FEF2F2] active:scale-90"
                                style="width:30px; height:30px; border-radius:50%; background:#fff; border:1.5px solid #ef4444; cursor:pointer; flex-shrink:0; -webkit-appearance:none; appearance:none;">
        ...
        <div style="max-width:900px; margin:0 auto; width:100%; display:flex; gap:8px;">
            <button @click="staged = {}; showProductos = false" type="button"
                    class="transition-all duration-150 hover:bg-gray-50 hover:border-gray-300 active:scale-95"
            ...
            <button type="button"
                    ...
                    :class="stagedTotal() === 0 ? '' : 'transition-all duration-150 hover:brightness-110 hover:shadow-lg active:scale-95'"
                    @click="
                        const calls = Object.entries(staged).filter(([id, q]) => q > 0).map(([id, q]) => $wire.agregar(parseInt(id), q));
                        Promise.all(calls).then(() => { staged = {}; showProductos = false; });
                    "
                    :style="stagedTotal() === 0 ? 'flex:1; padding:11px; border-radius:10px; font-size:13px; font-weight:700; color:#9CA3AF; background:#F3F4F6; border:none; cursor:not-allowed; display:flex; align-items:center; justify-content:center; gap:6px; -webkit-appearance:none; appearance:none;' : 'flex:1; padding:11px; border-radius:10px; font-size:13px; font-weight:700; color:#fff; background:#7B6FE8; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:6px; -webkit-appearance:none; appearance:none;'">
                <svg width="14" height="14" fill="none" :stroke="stagedTotal() === 0 ? '#9CA3AF' : '#fff'" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                <span x-text="stagedTotal() === 0 ? 'Confirmar' : 'Confirmar — ' + stagedTotal() + (stagedTotal() === 1 ? ' artículo' : ' artículos')"></span>
            </button>
        </div>
        </div>
    ...
    <div style="background:#fff; border-radius:20px; width:100%; max-width:560px; max-height:90vh; display:flex; flex-direction:column; box-shadow:0 24px 60px rgba(60,52,137,0.18), 0 0 0 1px rgba(196,181,253,0.15); position:relative;">
        ...
        <div style="display:flex; gap:8px; padding:14px 20px; border-top:1px solid #F0EEFF; flex-shrink:0;">
            ...
            <button wire:click="guardarNuevoCliente" wire:loading.attr="disabled"
                    style="flex:1; padding:11px; border-radius:10px; font-size:13px; font-weight:700; color:#fff; background:#7B6FE8; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:6px; box-shadow:0 2px 10px rgba(123,111,232,0.30);">
