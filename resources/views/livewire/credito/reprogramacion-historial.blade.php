        ...
        <div x-data="{ abierto: false }" style="margin-top:14px; border:1px solid #E5E7EB; border-radius:10px; overflow:hidden;">
            ...
            <div x-show="abierto" x-collapse>
                <div style="border-top:1px solid #E5E7EB;">
                    <div style="background:#F8F7FF; display:grid; padding:8px 12px; grid-template-columns:auto auto 1fr 1fr 1fr; gap:12px;">
                        <p style="font-size:10px; font-weight:600; color:#6b7280; margin:0; text-align:center;">#</p>
                        <p style="font-size:10px; font-weight:600; color:#6b7280; margin:0; text-align:center;">Cuotas</p>
                        <p style="font-size:10px; font-weight:600; color:#6b7280; margin:0; text-align:center;">Monto</p>
                        <p style="font-size:10px; font-weight:600; color:#6b7280; margin:0; text-align:center;">Vencimiento</p>
                    ...
                    @php $badge = $c->estadoFinancieroBadge; @endphp
                    <div style="{{ !$loop->last ? 'border-bottom:0.5px solid #e5e7eb;' : '' }}{{ $c->estado==='pagado' ? 'opacity:0.6;' : '' }} display:grid; align-items:center; padding:8px 12px; grid-template-columns:auto auto 1fr 1fr 1fr; gap:12px;">
                        <span style="font-size:11px; color:#374151; text-align:center; display:block;">{{ $c->numero }}</span>
                        <span style="font-size:11px; color:#6b7280; font-weight:600; text-align:center; display:block;">Cuota {{ $c->numero }}</span>
                        <span style="font-size:13px; font-weight:700; color:#9CA3AF; text-align:center; font-family:monospace; display:block;">{{ number_format($c->monto, 2) }}</span>
                        <span style="font-size:11px; color:#6b7280; text-align:center; display:block;">{{ $c->fecha_vencimiento ? \Carbon\Carbon::parse($c->fecha_vencimiento)->format('d/m/Y') : '—' }}</span>
                        <span style="display:flex; justify-content:center;">
                            <span class="ds-badge" style="background:{{ $badge['bg'] }};color:{{ $badge['cl'] }};">{{ $badge['lb'] }}</span>
                        </span>
                    </div>
