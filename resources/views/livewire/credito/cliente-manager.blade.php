...
<div class="sm:hidden flex flex-col" style="gap:10px;">
    ...
    <div wire:key="card-{{ $c->id }}"
        ...
        <div style="padding:10px 14px; display:flex; gap:12px; align-items:center; flex-wrap:wrap;">
            ...
            <span style="font-size:12px; color:#6B7280;"><span style="font-weight:600; color:#9CA3AF;">Vendedor:</span> {{ $c->vendedorUsuario->name }}</span>
            @endif
        </div>
        ...
        <table style="table-layout:fixed; width:100%; min-width:900px; border-collapse:collapse; font-size:13px;">
            ...
            <tbody>
                ...
                <tr wire:key="c-{{ $c->id }}"
                    ...
                    <td style="padding:10px 8px; text-align:center; font-size:11px; font-weight:700; color:#7B6FE8; white-space:nowrap; background:#F9F8FF;">{{ $clientes->firstItem() + $loop->index }}</td>
