...
<head>
    ...
    <style>
        ...
        .sidebar-nav::-webkit-scrollbar-thumb { background: #E7E0DC; border-radius: 4px; }
    </style>
    @livewireStyles
</head>
...
<div class="flex h-screen overflow-hidden">
    ...
    <div x-show="sidebarOpen"
         ...
         style="background:rgba(55,65,81,.4);"></div>
    ...
    <aside class="-translate-x-full md:translate-x-0
           ...
           style="background:#fff; border-right:1px solid #E7E0DC;"
           :class="{ 'translate-x-0': sidebarOpen }">

        {{-- Logo / Marca --}}
        <div style="padding:18px 20px; border-bottom:1px solid #F3EDE9; background:#FFF1F2; flex-shrink:0;">
            <div style="display:flex; align-items:center; gap:12px;">
                <div style="width:38px; height:38px; background:#BE185D; border-radius:10px; display:flex; align-items:center; justify-content:center; flex-shrink:0; box-shadow:0 4px 12px rgba(190,24,93,.25);">
                ...
                <div>
                    <p style="font-size:18px; font-weight:800; color:#374151; letter-spacing:2px; line-height:1;">ESSEN</p>
                    <p style="font-size:9px; color:#BE185D; font-weight:700; letter-spacing:1.5px; text-transform:uppercase; margin-top:3px; opacity:.7;">{{ $navUser->getRoleNames()->first() }}</p>
                </div>
            </div>
        </div>
        ...
        <div style="padding:10px 12px 4px; flex-shrink:0;">
            <a href="{{ route('administrativo.dashboard') }}"
               ...
               style="padding:10px 12px; {{ $dashActivo
                   ? 'background:#FFF1F2; color:#BE185D; font-weight:600; border-left:3px solid #BE185D;'
        ...
        <nav class="sidebar-nav flex-1 overflow-y-auto"
            ...
            <div style="margin-bottom:2px;">
                ...
                <button @click="activeModule = (activeModule === '{{ $slug }}' ? '' : '{{ $slug }}')"
                        class="w-full flex items-center gap-3 rounded-xl text-sm transition-all"
                        style="padding:10px 12px; font-weight:500;"
                        :style="activeModule === '{{ $slug }}'
                            ? 'background:#FFF1F2; color:#BE185D; font-weight:600; border-left:3px solid #BE185D;'
                            : 'color:#6B7280; border-left:3px solid transparent; background:transparent;'">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-show="activeModule === '{{ $slug }}'"
                     ...
                     style="margin-left:14px; padding-left:10px; border-left:2px solid #F3EDE9; margin-top:2px;">
                    ...
                    <div x-data="{ subOpen: {{ $grupActivo ? 'true' : 'false' }} }" style="margin-bottom:1px;">
                        <button @click="subOpen = !subOpen"
                                class="w-full flex items-center gap-2 rounded-lg text-sm transition-all"
                                style="padding:8px 10px; {{ $grupActivo ? 'color:#BE185D; font-weight:600; background:#FFF1F2;' : 'color:#9CA3AF; font-weight:500;' }}">
                        ...
                        <div x-show="subOpen"
                             style="margin-left:10px; padding-left:8px; border-left:2px solid #F9F0EE; margin-top:1px;">
                            ...
                            @endphp
                            <a href="{{ $href }}"
                               class="flex items-center gap-2 rounded-lg text-xs transition-all"
                               style="padding:7px 10px; margin-bottom:1px; {{ $childActivo ? 'color:#BE185D; font-weight:600; background:#FFF1F2;' : 'color:#9CA3AF;' }}">
                                ...
                                <span class="w-1 h-1 rounded-full flex-shrink-0"
                                      style="background:{{ $childActivo ? '#BE185D' : '#D4C5BF' }};"></span>
                                @endif
                                {{ $child->name }}
                            </a>
                            @endforeach
                    ...
                    @endphp
                    <a href="{{ $href }}"
                       class="flex items-center gap-2.5 rounded-lg text-sm transition-all"
                       style="padding:8px 10px; margin-bottom:1px; {{ $subActivo ? 'color:#BE185D; font-weight:600; background:#FFF1F2;' : 'color:#9CA3AF;' }}">
                        @if(isset($subIconos[$sub->slug]))
                        <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $subIconos[$sub->slug] }}"/>
                        </svg>
                        ...
                        <span class="w-1.5 h-1.5 rounded-full flex-shrink-0"
                              style="background:{{ $subActivo ? '#BE185D' : '#D4C5BF' }};"></span>
                        @endif
                        {{ $sub->name }}
                    </a>
                    @endif
            ...
            <p class="px-3 py-6 text-xs text-center" style="color:#C4B8B2;">Sin módulos asignados.</p>
            @endif
        </nav>

        {{-- Usuario + Logout --}}
        <div style="padding:14px 16px; border-top:1px solid #F3EDE9; background:#FAF7F5; flex-shrink:0;">
            <div style="display:flex; align-items:center; gap:10px; margin-bottom:10px;">
                <div style="width:34px; height:34px; border-radius:50%; background:#FFF1F2; border:2px solid #FCA5A5; display:flex; align-items:center; justify-content:center; font-size:13px; font-weight:700; color:#BE185D; flex-shrink:0;">
                    {{ strtoupper(substr($navUser->name, 0, 1)) }}
                </div>
                <div style="flex:1; min-width:0;">
                    <p style="font-size:13px; font-weight:600; color:#374151; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{{ $navUser->name }}</p>
                    <p style="font-size:10px; color:#A8A29E; text-transform:capitalize;">{{ $navUser->getRoleNames()->first() }}</p>
                </div>
            </div>
            <form method="POST" action="{{ route('logout') }}">
                ...
                <button type="submit"
                        ...
                        style="padding:9px; background:#FEE2E2; color:#DC2626; border:none; cursor:pointer; font-family:'Inter',sans-serif;">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header class="flex items-center gap-3 px-4 flex-shrink-0"
                style="background:#fff; border-bottom:1px solid #E7E0DC; min-height:56px;">
            <button @click="sidebarOpen = !sidebarOpen"
                    class="md:hidden flex items-center justify-center rounded-xl transition-colors"
                    style="min-width:40px; min-height:40px; color:#BE185D; background:#FFF1F2; border:none; cursor:pointer;">
            ...
            <div class="flex-1 flex items-center gap-2 min-w-0">
                @if($activeModuloName)
                <span style="font-size:12px; color:#C4B8B2; font-weight:500;">{{ $activeModuloName }}</span>
                <span style="color:#E7E0DC; font-size:12px;">/</span>
                @endif
                <h1 style="font-size:14px; font-weight:600; color:#374151; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                    @yield('page-title', 'Panel')
                </h1>
            </div>

            <div style="font-size:12px; color:#C4B8B2; display:none;" class="sm:block">
                {{ now()->format('d/m/Y') }}
            </div>
        </header>
